...
int_10_pow_19_top: .word 0x8ac72304 # 10^19. Used for print result in decimal
int_10_pow_19_bot: .word 0x89e80000 # 10^19. Used for print result in decimal
dec_result: .asciiz "Result in dec: "
minus_sign: .asciiz "- ("
closing: .asciiz ")"
hex_result: .asciiz "Result in hex: \n"
space: .asciiz "   "
enter: .asciiz "\n"
...
jal divide_128_64_unsigned # divide A*B to 10^19 to get 64 bit int_dec_result_01 and remainder
add $s6,$v0,$zero
add $s7,$v1,$zero

#store 64 bit in in_dec_result
sw $s6,int_dec_result_0
sw $s7,int_dec_result_1

#Start divide remainder to 10^18
...
#Start divide remainder to 10^9
lw $t3,int_10_pow_9
add $t2,$zero,$zero
jal divide_128_64_unsigned
add $s6,$v0,$zero
...
syscall
j end
add_128_bit_unsigned: # add 128 bit a0-a3 with 128 bit t0-t3 and save in a0-a3
	add_3: # Cộng t3 và a3
...
multiply_64_64_unsigned: # multiply 64 bit A in $t2-$t3 and 64 bit B in $t4-$t5
	#Adjust-Stack
...
divide_128_64_unsigned: #divide 128 bit A (a0-a3) to 64 bit B (t2-t3), the result is store in $v0,$v1 and the remainder is in t2-t3
	#Adjust-Stack
	#Lưu địa chỉ trả về trước vì sử dụng các hàm khác
	addi $sp, $sp, -4 # adjust stack for 1 item
	sw $ra, 0($sp) # save return address
	...
	addi $sp, $sp, 4 # pop 1 item from stack
	jr $ra
sub_128_bit_unsigned: # sub 128 bit a0-a3 with 128 bit t0-t3 and save in a0-a3 (a>t)
